...
void Player::update(float dtime)
{
    ...
    sf::Color c = playerShape.getFillColor();
    if (c != sf::Color::Blue && !m_isDodging && !isBloocking)
    {
    c.r = std::min<int>(c.r + 200 * dtime, 255);
    c.g = std::min<int>(c.g + 200 * dtime, 255);
    c.b = std::min<int>(c.b + 200 * dtime, 255);
    playerShape.setFillColor(c);
    }
...
void Player::takeDamage(float dmg)
    {
        ...
        std::cout << "Jugador recibió " << dmg << " daño. HP = " << currentHealth << "\n";

        playerShape.setFillColor(sf::Color(255, 100, 100));
    }